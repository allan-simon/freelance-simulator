...
allowed-tools: Bash(node ${CLAUDE_PLUGIN_ROOT}/cli.js *), Read, Grep, Agent
...
Tu as accès à un simulateur financier complet via `node ${CLAUDE_PLUGIN_ROOT}/cli.js`. C'est ta source de vérité pour tous les chiffres. **Ne fais JAMAIS de calcul mental** — lance toujours le CLI.

### Commandes disponibles

```bash
# Exploration step-by-step (chaque step montre les ranges pour la suivante)
node ${CLAUDE_PLUGIN_ROOT}/cli.js --step1 --tjm 1200 --jours 220
node ${CLAUDE_PLUGIN_ROOT}/cli.js --step2 --tjm 1200 --jours 220
node ${CLAUDE_PLUGIN_ROOT}/cli.js --step3 --tjm 1200 --jours 220 --salaireBrut 60000 --per 5000 --divNetsVoulus 40000
node ${CLAUDE_PLUGIN_ROOT}/cli.js --step4 --tjm 1200 --jours 220 --salaireBrut 60000 --per 5000 --divNetsVoulus 40000 --rendement 0.06 --ageObjectif 50

# Rapport complet (steps 1→4)
node ${CLAUDE_PLUGIN_ROOT}/cli.js --step4 [tous les params]

# JSON pour traitement programmatique
node ${CLAUDE_PLUGIN_ROOT}/cli.js --json [tous les params]
...
Quand l'utilisateur pose une question, lance d'abord `node ${CLAUDE_PLUGIN_ROOT}/cli.js --step4` avec ses paramètres (ou les défauts) pour avoir la baseline. Montre les chiffres clés.
